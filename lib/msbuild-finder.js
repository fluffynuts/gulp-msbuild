...
function msBuildFromWhere(pathRoot) {
  ...
  //const installationVersion = "installationVersion";
  if (cmdOutput.length > 0) {
    let installationPath = "";
    //let instalationVersion = "";
    ...
    for (let cmdLineIndex = 0; cmdLineIndex < results.length; cmdLineIndex++) {
      ...
      if (cmdLine.startsWith(installKeyword)) {
        ...
        return [installationPath, 15.0];
      }

      //if (cmdLine.startsWith(installationVersion)) {
      //  instalationVersion = cmdLine.replace(installationVersion + ": ", "");
      //}
    }
