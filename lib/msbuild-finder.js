...
const detectMsBuildOverXBuild = function () {
  try {
    const
      output = child.spawnSync("which", [ "msbuild" ], { encoding: "utf8" });
    if (output.status !== 0) {
      return "xbuild";
    }
    return "msbuild";
...
function lsR(folder) {
  ...
  lsCache[folder] = fs.readdirSync(folder)
    .reduce((acc, cur) => {
...
module.exports.find = function (options) {
  ...
  if (!version) {
    throw new PluginError({
      ...
      message: "No or invalid MSBuild version was supplied! Please set or check the value of the 'toolsVersion' property."
    });
  }
